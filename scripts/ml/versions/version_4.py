#!/usr/bin/env python3
"""
Version 4: Object-Oriented Custom Backward Selection with Aesthetic CV Spinners
...
- Aesthetic CV progress spinners: ⬜⬛ for CV progress (3-fold = ⬜⬜⬛ showing 2 done, 1 current)
- Same defaults: CV=3, removal_ratio=0.1, tuning=True
- Command: python version_4.py -c titanic
"""
...
class CustomBackwardSelector:
    """Custom backward feature selection with CV inside each iteration (OOP version of Version 2)."""
...
class CVEvaluator:
    """Simplified CV evaluator for baseline models."""
    
    def __init__(self, cv_folds: int = 3, random_state: int = 42):
        self.cv_folds = cv_folds
        self.random_state = random_state
    
    def cross_validate_simple(self, df: pd.DataFrame, target: str, 
        ...
        mean_score, _, _, _ = selector._train_model_with_cv(
            df, 
            [col for col in df.columns if col != target], 
            0
        )
        
        return mean_score, 0.0  # Return score and 0 std


class OldCVEvaluator:
    """Legacy class - keeping for compatibility but not used."""
    
    def train_silently(self, learner, train_data, valid_data=None):
        """Train YDF model silently."""
        import tempfile
        import os
        import subprocess
        import sys
        ...
        model_path = log_path.replace('.log', '_model')
...
"""
            ...
            for path in [train_path, valid_path, learner_path, script_path, log_path]:
    ...
    def tune_hyperparameters(self, train_df: pd.DataFrame, valid_df: pd.DataFrame, 
                           target: str, model_type: str, problem_type: str, 
                           n_trials: int = 20) -> Dict[str, Any]:
        """Tune hyperparameters using validation set."""
        console.print(f"      → Tuning hyperparameters ({n_trials} trials)...")
        
        task = self.determine_task(problem_type)
        ...
        if model_type == 'gbt':
            ...
            search_space = {
                ...
                'min_examples': [5, 10, 20],
                'num_candidate_attributes_ratio': [0.5, 0.7, 1.0, -1.0]
            }
        ...
        for trial in range(n_trials):
            ...
            try:
                ...
                learner = self.create_learner(model_type, target, task, **params)
                model = self.train_silently(learner, train_df, valid_df)
                ...
                if needs_probabilities('roc_auc') and 'classification' in problem_type:
                    if problem_type == 'binary_classification':
                        if hasattr(predictions, 'probability'):
                            y_pred = predictions.probability(1)
                        else:
                            y_pred = predictions
                        score = calculate_metric(y_true, y_pred, 'roc_auc', problem_type)
                    else:
                        # For multiclass, use accuracy
                        y_pred = predictions
                        score = calculate_metric(y_true, y_pred, 'accuracy', problem_type)
                else:
                    y_pred = predictions
                    if problem_type == 'regression':
                        score = calculate_metric(y_true, y_pred, 'rmse', problem_type)
                    else:
                        score = calculate_metric(y_true, y_pred, 'accuracy', problem_type)
                
                # Check if better
                is_better = (
                    ('classification' in problem_type and score > best_score) or
        ...
        if best_params:
            ...
            console.print(f"      → No valid hyperparameters found, using defaults")
        
        return best_params


class MDMBenchmark:
    """Main benchmark class with object-oriented design."""
    ...
    def __init__(self, output_dir: str = "benchmark_results", use_cache: bool = True):
        ...
        self.results = {
            'version': 'Version 4: Object-Oriented Custom Backward Selection with Aesthetic CV Spinners',
            'description': 'Custom backward selection with CV inside (3-fold) + tuning, returns CV score as final result',
    ...
    def benchmark_competition(self, name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """Benchmark a single competition using object-oriented approach."""
        ...
        console.print("\n[bold]Training models with Object-Oriented Custom Selection...")
        model_types = ['gbt', 'rf']
        ...
        for model_type in model_types:
            ...
            try:
                cv_evaluator = CVEvaluator(cv_folds=3, random_state=42)
                
                mean_score, std_score = cv_evaluator.cross_validate_simple(
                    df_raw, 
                    config['target'],
                    model_type,
                    config['problem_type'],
    ...
    def run_benchmark(self, competitions: Optional[List[str]] = None):
        ...
        console.print(Panel.fit(
            f"[bold]Version 4: Object-Oriented Custom Backward Selection[/bold]\n"
            f"Custom backward selection with CV inside (3-fold) + tuning\n"
            f"Returns CV score from feature selection as final result\n"
            f"Aesthetic CV spinners: ⬜⬛ for progress visualization\n"
    ...
    def save_results(self):
        ...
        output_file = self.output_dir / f"v4_results_{timestamp}.json"
    ...
    def display_summary(self):
        """Display summary table with object-oriented formatting."""
        table = Table(title="Benchmark Summary - Version 4 (Object-Oriented)", show_header=True)
        ...
        for name, result in self.results['results'].items():
            ...
            table.add_row(name, status, features_str, gbt_imp, rf_imp, method)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Version 4: Object-Oriented Custom Backward Selection with Aesthetic CV Spinners"
    )
    parser.add_argument(
        '--competitions', '-c',
        nargs='+',
    ...
    if not HAS_YDF:
        console.print("[red]Error: YDF (Yggdrasil Decision Forests) is required but not installed.[/red]")
        console.print("Install with: pip install ydf")
        sys.exit(1)
